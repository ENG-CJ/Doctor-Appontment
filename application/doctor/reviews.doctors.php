            ...
            <div class="col-sm-6 p-md-0">
                <div class="welcome-text">
                    <div class="alert alert-warning mb-3">
                        <strong>
                            This data will be reviewed by system administrators for the purpose of ensuring client satisfaction.</strong>
                    </div>
                    <h4>My Reviews</h4>
                    <span class="ml-1">Here is a List of Reviews From Patients About Your Appointments</span>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        function countReviews(id, getResponse) {
            $.ajax({
                method: "POST",
                dataType: "JSON",
                data: {
                          ...
                          <div class="col-6">
                            <div class="card text-dark ${satisfied > unsatisfied ? "border-success" : "border-danger" } mb-3 w-100">
                                <div class="card-header ${satisfied > unsatisfied ? "border-success" : "border-danger" } d-flex justify-content-between align-items-center">
                                    ...
                                    <img src="../uploads/${value.profile_image}" alt="" class="img-fluid rounded-circle text-right" style='${satisfied > unsatisfied ? 'border: 1px solid green' : 'border: 1px solid red'}; width: 70px; height: 70px'>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="my-2">
                                        <span class="badge badge-pill badge-danger mr-2">unsatisfied Patients: ${unsatisfied}</span>
                                        <span class="badge badge-pill badge-success">satisfied Patients ${satisfied}</span>
                                    </div>
        ...
        // readSchedules()
        $(document).on("click", ".sendWarning", function() {
            var email = $(this).attr("drEmail");
            var drName = $(this).attr("drName");
            var messageEmail = $(".warnEmail").val();

            if ($(".warnEmail").val() == "") {
                swal("Provide Warning Message");
                return;
            }
            $.ajax({
                method: "POST",
                ...
                success: (res) => {
                    ...
                    if (res.error == "") {
                        swal("Email has been sent to this doctior", {
                            icon: "success",
                        });
                        $(".warnEmail").val("");
                    } else {
        ...
        $(document).on("click", ".ban", function() {
            ...
            swal({
                    title: "Bann This Doctor From The System",
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            ...
                            success: (res) => {
                                console.log(res)
                                swal("Doctor Has been banned from the system", {
                                    icon: "success",
                                });
                                readReviews();
                            },
